...
function updateCubeTransform() {
    ...
    if(moveFront || moveBack || boxZRotSpeed != 0) {
        ...
        // Refresh this object's position on the network
...
// TODO: delete object on close connection
}
...
function readIncomingMessage(e){ 
    ...
    if(data.type === 'user-init') {
        ...
        users.forEach((user => {
            if(user.id == data.id) {
                console.log('user ' + user.id + ' moved.')
                user.mesh.matrix.fromArray(data.matrix)
            }
        }))
    }
}
