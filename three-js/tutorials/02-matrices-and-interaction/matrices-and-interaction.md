...
Then, let's then a `Rotation` matrix. To do that, we can tell three.js to create a new `4 x 4` matrix from a rotation, which in this particular case is a rotation around the `z-axis`. There are two was of doing it: making the matrix from a rotation around an axis by calling [`THREE.matrix4().makeRotationAxis(axis, theta)`](https://threejs.org/docs/index.html#api/en/math/Matrix4.makeRotationAxis), which takes a `THREE.Vector3` as the axis and an angle in radians. The second way uses *Quaternions* and makes the rotation amtrix based on a [quaternion](https://en.wikipedia.org/wiki/Quaternion), which is made from a rotation around an axis. This may sound like overdoing right now, but is the most precise way of amking rotations in 3D, since quaternions represent rotations more accurately than Euler angles (refer to the video [Quaternions and 3d rotation, explained interactively](https://www.youtube.com/watch?v=zjMuIxRvygQ) for further information about quaternions).
