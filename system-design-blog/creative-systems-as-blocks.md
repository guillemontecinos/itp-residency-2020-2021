...
Then, the heart beat rate is used to generate a pulse-like electrical signal that dims the bulbs array simulating the spatialization of the heart beat. This terciary step –that conforms the system's main process– runs on a cumputer and outputs not a single signal, but an array of signals that control each bulb.
